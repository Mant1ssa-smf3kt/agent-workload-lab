...
> config 的 concurrency=2；本估计按 c=1、不驱逐算，并发下的驱逐与排队只能上 GPU（docs/findings.md §6）。

| 组 | cache hit（估计） | 上界（缓存无限） | 每请求命中 P50 / P95 / P99 | 命中 < 0.5 的请求 | prompt tok 总 | n |
|---|---|---|---|---|---|---|
| w4-c2 | 0.9633 | 0.9633 | 0.979 / 0.996 / 0.998 | 4.8% | 19.38M | 837 |
| w4-c2（不含合成） | 0.9633 | 0.9633 | 0.979 / 0.996 / 0.997 | 4.8% | 19.36M | 835 |
| w4-c1（对照） | 0.9633 | 0.9633 | 0.979 / 0.996 / 0.998 | 4.8% | 19.38M | 837 |
| Δ（w4-c2 − w4-c1） | +0.0000 (+0.0%) | | | | +0 | |
