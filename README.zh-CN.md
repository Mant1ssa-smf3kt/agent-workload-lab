...
「system prompt 里放动态内容会破坏前缀缓存」不是新知识——各家 prompt caching 文档都要求静态内容放前面。本仓库补上的是**它在负载下的代价，以及为什么**：

- **缓存失效的代价取决于并发，单会话基准会低估它。** 同一个时间戳，1 条会话时单轮 P95 +7.7%，4 条会话同时跑时 +36.7%。修法没有成本。
- **对 agent 负载，KV 池本质上是会话缓存；过了它的容量，加并发反而降吞吐。** 回撤罕见，压力由驱逐空闲会话的前缀吸收。悬崖在 2 与 4 条会话之间，与「会话数 × 上下文 ≈ KV 池」（这里 78k token）一致。c=8 做同样的活比 c=4 多花 16% 墙钟，三次重跑共 11 个请求超时。容量规划应按这个乘积做准入控制，而不是按算力加并发。
