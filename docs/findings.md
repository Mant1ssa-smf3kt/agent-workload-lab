...
> 2026-09-23 更正：本节原标题「回撤一次都没发生」是错的。原依据 `sglang:num_retracted_reqs` 是 gauge，每个统计周期上报后清零，快照上几乎总是 0；累计回撤数在 `sglang:num_retracted_requests_total`（`docs/decisions.md` 2026-09-23）。

按累计计数器的 run 前后差：c1、c2 为 0；c4 每 run 1 / 4 / 4 次，c4-timestamp 2 / 2 / 1，c8 1 / 2 / 0——每 run 837 个请求里最多 4 个。被回撤请求的输入 token 三次合计 c4 237k、c4-timestamp 141k、c8 79k，分别是同组驱逐 token 的 1.6% / 0.26% / 0.28%（`experiments/w4-*/report.md` 的 `retracted requests` / `evicted tokens` 行，原始值在 `out/*/metrics_{before,after}.json`）。原因在负载形状：输出 P50 只有 114 token，运行中请求的 KV 增长可忽略；池子里塞满的是**空闲会话的缓存前缀**（工具执行间隙里的那些）。调度器绝大多数时候有可驱逐的叶子，回撤只在少数时刻出现。agent 负载下 KV 池实际上是一个会话缓存，决定尾延迟的主要是 LRU 驱逐，不是为回撤准备的那些旋钮。
