...
2. **把录制模型的上下文窗口压到与重放侧一致**（`scripts/pi-models.recording.json` → 合并进 `~/.pi/agent/models.json`）。
   pi 的 compaction 触发条件是 `contextTokens > contextWindow − 16384`（`reserveTokens` 默认值）。glm-5.3 的窗口是 1M，
...
```bash
cd <目标仓库>
pi --model zai/glm-5.3:off        # thinking off，见下
```

- **thinking 关掉录第一批。** 开着会把 `reasoning_content` 写回后续轮次的 assistant 消息（zai 走 `clear_thinking: false`），prompt 形状会和小模型重放时明显不同。要研究 reasoning 对上下文增长的影响，另起一批、单独标注。
